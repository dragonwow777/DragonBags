...
-- Set to true to print diagnostic messages to chat.
...
function addon:Debug(...)
	...
	print("|cff33ff99DragonBags DEBUG:|r", msg)
end
            ...
            for slot = 1, numSlots do
                ...
                if itemID then
                    ...
                    if entry then
                        ...
                        local name, _, quality, iLevel, _, class, subclass, maxStack, _, texture = GetItemInfo(itemID)
                        inventory[itemID] = {
                            count    = count,
                    ...
                    self:Debug("Found itemID " .. itemID .. " in bag " .. bagID .. ", slot " .. slot .. ", count " .. count)
                end
            end
        end
    end
...
function addon:ScanBankOnly()
    ...
    local function addSlot(containerID, slot)
        local itemID = GetContainerItemID(containerID, slot)
        if not itemID then return end
        local _, count = GetContainerItemInfo(containerID, slot)
        count = count or 1
        local entry = inventory[itemID]
        if entry then
            entry.count = entry.count + count
...
function addon:SaveBankData(event, ...)
	...
	if not self.db.global.characters[characterKey] then self.db.global.characters[characterKey] = {} end
	self.db.global.characters[characterKey].bank = self:ScanBankOnly()
...
function addon:SaveCharacterData()
    ...
    if not self.db.global.characters[characterKey] then self.db.global.characters[characterKey] = { bags = {}, bank = {} } end
    local _, class = UnitClass("player")
    self.db.global.characters[characterKey].class = class
    local money = GetMoney()
    self.db.global.characters[characterKey].money = money
    self.db.global.characters[characterKey].lastUpdate = time()
    local bags = self:ScanBagsOnly()
    self.db.global.characters[characterKey].bags = bags
    self:Debug("Saved bags and money for", characterKey)
end

function addon:OnPlayerLogin()
    self:UnregisterEvent("PLAYER_LOGIN")
    print("|cff33ff99DragonBags:|r Scheduling initial character data scan.")
...
function addon:OnInitialize()
	...
	-- >>> NEW CODE: Load and initialize the Bank module using global access.
    local Bank = _G.Bank
    if Bank and Bank.Init then
        Bank.Init(self)
        self.Bank = Bank
		-- Manually trigger OnBagFrameCreated for Backpack
        for index, bag in self:IterateBags() do
            if bag:GetFrame().name == "Backpack" then
                self.Bank:OnBagFrameCreated(bag)
            end
        end
    end
	-- <<< END NEW CODE
	...
	-- Persistant handlers
	self.RegisterBucketMessage(addonName, 'DragonBags_ConfigChanged', 0.2, function(...) addon:ConfigChanged(...) end)
	
	self.RegisterEvent(addonName, 'PLAYER_ENTERING_WORLD', function() if self.db.profile.enabled then self:Enable() end end)

	self:UpgradeProfile()

	-- Register PLAYER_LOGIN here so it fires on first load.
	-- OnEnable is triggered by PLAYER_ENTERING_WORLD (which fires after PLAYER_LOGIN),
	-- so registering there would always miss it on new sessions.
	...
	if PV then
		...
		self.RegisterMessage(PV, "DragonBags_UpdateButton", function(_, button)
...
function addon:OnEnable()
    self:EnableModule("Finance")
    self.globalLock = false
    self:RegisterEvent('BAG_UPDATE')
    self:RegisterBucketEvent('PLAYERBANKSLOTS_CHANGED', 0, 'BankUpdated')
    self:RegisterMessage('DragonBags_BagOpened', 'LayoutBags')
    self:RegisterMessage('DragonBags_BagClosed', 'LayoutBags')
    ...
    self:RegisterEvent("PLAYER_LOGOUT", function()
        -- Do NOT call SaveCharacterData here: at PLAYER_LOGOUT, WoW has already
        -- unloaded bag/money data, so GetMoney() returns 0 and ScanBagsOnly()
        -- returns empty — calling it would overwrite the good data saved by the
        -- BAG_UPDATE bucket during the session.
        self:Disable()
    end)
    ...
    for name, module in self:IterateModules() do
        ...
        elseif module.isBag then
            ...
            -- Use the AceAddon module name (loop variable 'name'), not
            -- module.moduleName which is only set on filter/bag modules and is
            -- nil here, causing SetEnabledState(nil) = always disabled.
...
function addon:BAG_UPDATE(event, bag) self:SendMessage('DragonBags_BagUpdated', bag) end

function addon:BankUpdated(slots)
	for slot in pairs(slots) do
		if slot > 0 and slot <= NUM_BANKGENERIC_SLOTS then self:SendMessage('DragonBags_BagUpdated', BANK_CONTAINER); return end
	end
end
...
function addon:ConfigChanged(vars)
	...
	elseif vars.filter then return self:SendMessage('DragonBags_FiltersChanged')
	else
		for name in pairs(vars) do
			if strmatch(name, 'virtualStacks') then return self:SendMessage('DragonBags_FiltersChanged')
			...
			elseif strmatch(name, 'rowWidth') then return self:SendMessage('DragonBags_LayoutChanged')
	...
	elseif vars.oneBagSortingOrder then self:SendMessage('DragonBags_LayoutChanged')
	elseif vars.maxHeight or vars.laxOrdering then return self:SendMessage('DragonBags_LayoutChanged')
	elseif vars.scale then return self:LayoutBags()
	elseif vars.positionMode then return self:UpdatePositionMode()
	else self:SendMessage('DragonBags_UpdateAllButtons') end
end

function addon:SetGlobalLock(locked)
	locked = not not locked
	if locked ~= self.globalLock then
		self.globalLock = locked; self:SendMessage('DragonBags_GlobalLockChanged', locked)
		if not locked then self:SendMessage('DragonBags_LayoutChanged') end
		return true
	end
end
	...
	function addon:UpdateInteractingWindow(event, ...)
		...
		if new ~= current then
			...
			if self.db.profile.virtualStacks.notWhenTrading then
				self:SendMessage('DragonBags_FiltersChanged', 0)
			end
			self:SendMessage('DragonBags_InteractingWindowChanged', new, old)
			self:SendMessage('DragonBags_TimeToCheckAnchorMode')
		end
	end
...
function bagProto:Open()
	...
	if not frame:IsShown() then
		self:Debug('Open')
		frame:Show()
		addon:SendMessage('DragonBags_BagOpened', self.bagName, self)
		return true
	end
end
...
function bagProto:Close()
	if self.frame and self.frame:IsShown() then
		...
		self.frame:Hide()
		CloseMenus()
		addon:SendMessage('DragonBags_BagClosed', self.bagName, self)
...
function bagProto:GetFrame()
	if not self.frame then
		...
		addon:SendMessage('DragonBags_BagFrameCreated', self)
	end
	return self.frame
end
...
function addon:HookBagFrameCreation(target, callback)
	...
	if listen then
		target:RegisterMessage("DragonBags_BagFrameCreated", hook)
	end
end
...
do
	...
	function backpack:DragonBags_InteractingWindowChanged(event, window)
		if window then
...
do
	...
	function bank:DragonBags_InteractingWindowChanged(event, new, old)
...
function addon:CreateBagAnchor()
	local anchor = self:CreateAnchorWidget(UIParent, "anchor", L["DragonBags Anchor"])
	...
	text:SetText(L["DragonBags Anchor"])
...
function addon:LayoutBags()
	...
	if self.db.profile.positionMode == 'anchored' then
		...
		self:SendMessage('DragonBags_AnchoredLayout')
	else
		ManualBagLayout(self)
		self:SendMessage('DragonBags_ManualLayout')
	end
end
...
function addon:ToggleCurrentLayout()
	...
	if self.db.profile.positionMode == 'anchored' then
		...
		-- First call with a duration of 0.5 seconds to remove delay of DragonBags message.
...
function addon:CycleViewMode()
	...
	self:SendMessage('DragonBags_ViewModeChanged')
	self:SendMessage('DragonBags_FiltersChanged', true) -- 'true' forces a full redraw
end
